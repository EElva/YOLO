...
#	threshold = 0.2
	...
	def detect_from_cvmat(self,img):
		...
		img_resized = imresize(img, (448, 448))
...
#		img = cv2.imread(filename)
	...
	def show_results(self,img,results):
		img_cp = img.copy()
		print( "Image size = ", img_cp.shape )
		if self.filewrite_txt :
			ftxt = open(self.tofile_txt,'w')

		draw = ImageDraw.Draw( self.jimg )
		...
		for i in range(len(results)):
			...
			if self.filewrite_img or self.imshow:
				...
				draw.rectangle( [ x-w,y-h,x+w,y+h ], outline = 'green' )
				draw.rectangle( [ x-w-1,y-h-1,x+w+1,y+h+1 ], outline = 'green' )
				draw.rectangle( [ x-w-2,y-h-2,x+w+2,y+h+2 ], outline = 'green' )
				draw.rectangle( [ x-w,y-h-20,x+w,y+h ], outline = 'green' )
				draw.rectangle( [ x-w-1,y-h-20-1,x+w+1,y+h+1 ], outline = 'green' )
				draw.rectangle( [ x-w-2,y-h-20-2,x+w+2,y+h+2 ], outline = 'green' )
				print( results[i][0] )
				draw.text( [x, y-h-7], results[i][0] )
				print( "OUT=", x, y, w, h )
			if self.filewrite_txt :				
				ftxt.write(results[i][0] + ',' + str(x) + ',' + str(y) + ',' + str(w) + ',' + str(h)+',' + str(results[i][5]) + '\n')
		if ( self.label_image == 0 ):
			o = 9
		else:
			tkImage = ImageTk.PhotoImage( self.jimg )
			self.label_image.img = tkImage
			self.label_image.pack(side = tkinter.TOP, expand=True, fill=tkinter.BOTH)
			self.label_image.create_image( 320,180, image=tkImage )	
